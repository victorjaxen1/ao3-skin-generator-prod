...
| P18 | **The ornament controls, saved and read back** | Added and closed 18 Aug 2026 with §31. Three properties this product had never stored: `font-variant`, `letter-spacing` and `border-style: double` — plus a `content` string on a `> .heading` pseudo-element and, the one genuinely new selector shape, **`:has()` used to scope rather than to decorate**. All probe-legal, none observed | ✅ **18 Aug 2026 — passes, and the readback was diffed rather than read (§26c.2).** 38 of 38 sampled declarations came back **byte-identical**; the only difference in the whole stylesheet is AO3 re-wrapping `.wrapper:has(> table, > .meta)` across two lines at the comma inside the parens — §26b's known anomaly, now seen a second time. Small caps render on the page headings **and** the site title, ❖ sits either side of *Fandoms* in the theme's accent, and the double rule survives alongside an 18px radius. **`.blurb .header:has(ul.required-tags)` was stored and works**, which is the gutter fix confirmed on the page type that broke |
