...
| # | Task | Why | Where |
| --- | --- | --- | --- |
| 3 | **`stripCssComments()` in `ao3Css.ts`**, used by the lint and anything else that pattern-matches CSS | Failing to strip comments before matching has produced **four** wrong answers: `lintAo3Css` refusing a stylesheet whose comment said `@media`, two tests reading values out of comments, and the namespacing prototype missing seven rules | MASTER §6a |
